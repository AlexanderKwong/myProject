...
public class XdrLocationMerge implements IWriteLogCallBack
{
	...
	public XdrLocationMerge(Date statDate)
	{
	...
	public boolean run()
	{
		...
		{		
			//构造执行脚本
	        String path= MainModel.GetInstance().getExePath() + "/xdrlocation.pig";
	        ...
	        String xdrFilePath = String.format("/result/test/%s", format.format(statDate));
			...
			sshHelper.excuteCmd("/opt/pig-0.15.0/bin/pig -x mapreduce " + path, 2);
