...
public class FingerGrid
{
    ...
    public double GetPrint(MrSplice splice, RefInt ccount, CfgInfo cInfo, int maxcount, FingerN[][] necis)
    {
        ...
        for (Map.Entry<Integer, MrPoint> kp : splice.ncells.entrySet()) 
        {
            ...
            //4.所有邻区信号出现概率；
            frate = SetRate(frate, GetOccurRate(splice.cityid, rsrp, sg.rsrp, sg.buildingid, cInfo, sg.eci, kp.getValue().cell.pciarfcn, necis));
            if (frate == 0)
            {
                return 0;
            }
    ...
    private double GetRsrpRate(double rsrp, double simursrp)
    {
        ...
        {
            return (((10 - rdiff) * 80.0 / 6 + 20) / 100.0);
        }
        else if (rdiff > -4)
        {
            return 1;
        ...
        {
            return (((15 + rdiff) * 80.0 / 11 + 20) / 100.0);
        }
        else
        {
            return 0;
