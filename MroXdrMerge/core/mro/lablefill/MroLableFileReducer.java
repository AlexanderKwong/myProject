...
public class MroLableFileReducer
{
	...
	public static class MroDataFileReducer extends DataDealReducer<CellTimeKey, Text, NullWritable, Text>
	{
		...
		public void reduce(CellTimeKey key, Iterable<Text> values, Context context) throws IOException, InterruptedException
		{
			...
			else
			{
				...
				while (values.iterator().hasNext())
				{
					...
					// strs = value.toString().split(StaticConfig.DataSliper2 +
					// "|" + "\\|" + "|" + "\t", -1);
		...
		private void statMro(DT_Sample_4G tsam, SIGNAL_MR_All tTemp)
		{
			...
			{
				// 只输出哈尔滨
				if (tsam.flag.equals("MRO") && tsam.IMSI > 0 && MainModel.GetInstance().getCompile().Assert(CompileMark.OutAllSample))
				{
					curText.set(ResultHelper.getPutLteSampleSmp(tsam));
