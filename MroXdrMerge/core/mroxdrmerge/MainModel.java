...
import jan.util.StreamConfigure;
...
public class MainModel
{
	...
	public boolean loadConfig()
	{
		...
		else if (getCompile().Assert(CompileMark.Home))
		{
			LOG.info("read Home config... ");

			path_app = "app_Home.xml";
			path_adapter = "xdr_data_adapter_Home.conf";
		}
		...
		else if (getCompile().Assert(CompileMark.GuangXi))
		{
			LOG.info("read GuangXi config... ");

			path_app = "app_GuangXi.xml";
			path_adapter = "xdr_data_adapter_GuangXi.conf";
			path_event_adapter = "event_data_adapter_GuangXi.conf";
		}
		...
		{
			/*
			 * if (new File(getExePath() + "/config/" + path_app).exists()) {
			 * conf = new FileConfigure(getExePath() + "/config/" + path_app);
			 * conf.loadConfigure(); appConfig = new AppConfig(conf);
			 * 
			 * dataAdapterConfig = new DataAdapterConf();
			 * dataAdapterConfig.init(getExePath() + "/config/" + path_adapter);
			 * LocationParseService.setUrlConfig(getExePath() +
			 * "/config/urlconfig.txt"); } else
			 */
			{
				...
				try
				{
					...
					if (getCompile().Assert(CompileMark.BeiJing) || getCompile().Assert(CompileMark.NeiMeng)
							|| getCompile().Assert(CompileMark.HaErBin)
							|| getCompile().Assert(CompileMark.GuangXi)
							|| getCompile().Assert(CompileMark.GuangXi2)
							|| getCompile().Assert(CompileMark.NingXia)
							|| getCompile().Assert(CompileMark.YunNan)
							|| getCompile().Assert(CompileMark.QingHai))
					{
				...
				LOG.info("loading urlconfig.txt," + path_app + "," + path_adapter + "," + path_event_adapter + " successed!");
