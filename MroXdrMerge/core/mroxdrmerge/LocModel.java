package mroxdrmerge;

import java.io.File;
import java.io.InputStream;
import java.lang.reflect.Method;
import java.net.URL;
import java.net.URLClassLoader;
import java.util.List;

import org.apache.log4j.Logger;

//import cn.mastercom.protocol.URLStreamHandlerFactory;

import com.chinamobile.xdr.LocationInfo;


public class LocModel
{
	protected static Logger LOG = Logger.getLogger(LocModel.class);

	private static LocModel instance = null;

	public static void main(String[] args)
	{
		LocModel.GetInstance("1234567mastercom");
	}
	
	public static LocModel GetInstance(String pass)
	{
		if (instance == null)
		{
			strPass = pass;
			LOG.info("create LocModel instance.");	
			instance = new LocModel(); 				
		}
		else
		{
			//System.out.println("GetInstance thread:" + Thread.currentThread().getId());
		}
		return instance;
	}

	private LocModel()
	{
		//File file = new File(this.getClass().getProtectionDomain().getCodeSource().getLocation().getPath());		
		LoadCTLib();
	}

	public static List<LocationInfo> DecryptLoc(String requestType, String host, String url, String downlinkContent , String uplinkContent,boolean bHex, boolean bHexDownlink)
	{
		if(mtdDecryptLoc != null)
		{
			try
			{
				return (List<LocationInfo>)mtdDecryptLoc.invoke(null, requestType, host, url, downlinkContent , uplinkContent,bHex, bHexDownlink);
			}
			catch (Exception e)
			{
				System.out.println("");
			}
		}		
		return null;
	}
	
	static Method setUrlConfig = null;
	static Method mtdDecryptLoc = null;
	public static String strPass = "";
	
	private static void LoadCTLib()
	{
		if(mtdDecryptLoc != null)
		{
			//System.out.println("LoadCTLib thread:" + Thread.currentThread().getId());
			return;
		}
		//System.out.println("LoadCTLib thread:" + Thread.currentThread().getId());
		
		URL[] urls = new URL[1];
		try
		{
	//		URL.setURLStreamHandlerFactory(new URLStreamHandlerFactory());
		}
		catch (Exception e1)
		{
			e1.printStackTrace();
		}
		
		try
		{
			if(strPass!="")
			{
				urls[0] = new URL("me:jarfile:/lib/LocParthLib_new.dat"+"@DES$"+strPass) ;
			}
			else
			{
				urls[0] = new URL("me:jarfile:/lib/LocParthLib.jar") ;				
			}
		}
		catch (Exception e1)
		{
			e1.printStackTrace();
		}
		
		try {
			URLClassLoader loader = new URLClassLoader(urls);
			Thread.currentThread().setContextClassLoader(loader);
		} catch (Exception e) {
			System.out.println("LoadCTLib fail setContextClassLoader: " + e.getMessage());
		}
		
		try {
			Class<?> clazz = Class.forName("com.chinamobile.xdr.LocDecryptor", true, Thread.currentThread().getContextClassLoader());
		 
			setUrlConfig = clazz.getDeclaredMethod("setUrlConfig",InputStream.class);
			mtdDecryptLoc = clazz.getDeclaredMethod("DecryptLoc",String.class, String.class, String.class, String.class , String.class,boolean.class, boolean.class);
			//setUrlConfig.invoke(clazz,""); 
			
			List<LocationInfo> locinfo = (List<LocationInfo>)mtdDecryptLoc.invoke(null,"post","loc.map.baidu.com","http://loc.map.baidu.com/sdk.php"
    				,""
    				,"",true,true);
    		for(int i=0; i<locinfo.size(); i++)
    		{
    			System.out.println(locinfo.toString());
    		}
		} catch (Exception e) {
			System.out.println("LoadCTLib forName fail: " + e.getMessage());
		}
	}
	
	

}
