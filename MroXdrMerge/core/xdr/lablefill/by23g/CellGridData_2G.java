...
public class CellGridData_2G
{
	...
	private Map<GridItem, Stat_CellGrid_2G> gridDataMap;
	
   
	public CellGridData_2G(int cityID, int lac, int ci, int startTime, int endTime)
	{
		this.cityID = cityID;
		this.lac = lac;
		this.ci = ci;
		...
		gridDataMap = new HashMap<GridItem, Stat_CellGrid_2G>();
	}
	
	public Map<GridItem, Stat_CellGrid_2G> getGridDataMap()
	{
		return gridDataMap;
	}
	...
	public void dealSample(DT_Sample_2G sample)
	{
		...
		if (sample.ilongitude > 0 && sample.ilatitude > 0)
		{
			...
			Stat_CellGrid_2G lteGrid = gridDataMap.get(gridItem);
			if (lteGrid == null)
			{
				lteGrid = new Stat_CellGrid_2G();
	...
	{
		for (Stat_CellGrid_2G item : gridDataMap.values())
		{
