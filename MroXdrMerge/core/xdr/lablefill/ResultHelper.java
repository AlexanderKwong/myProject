...
import StructData.Stat_Cell_Freq;
import StructData.Stat_Grid_23G;
...
import StructData.Stat_UserAct_Cell;
import StructData.Stat_UserGrid_4G;
...
public class ResultHelper
{
	...
	public static String getPutLteSample(DT_Sample_4G item)
	{
		...
		sb.append(item.ilatitude);
		sb.append(TabMark);
		sb.append(item.ispeed);
		sb.append(TabMark);
		sb.append(item.imode);
		sb.append(TabMark);
		sb.append(item.iLAC);
		sb.append(TabMark);
		sb.append(item.iCI);
		...
		sb.append(item.getEncryptIMSI());
		sb.append(TabMark);
		sb.append(item.getEncryptMSISDN());
		sb.append(TabMark);
		...
		{
			sb.append(item.ttds[i].TdsPccpchRSCP);
			sb.append(TabMark);
			sb.append(item.ttds[i].TdsNcellUarfcn);
			sb.append(TabMark);
			sb.append(item.ttds[i].TdsCellParameterId);
			sb.append(TabMark);
		}
		...
		{
			sb.append(item.trip[i].Earfcn);
			sb.append(TabMark);
			sb.append(item.trip[i].SubFrame);
			sb.append(TabMark);
			sb.append(item.trip[i].LteScRIP);
			sb.append(TabMark);
		}
		sb.append(item.LocFillType);
		sb.append(TabMark);
		...
		if (MainModel.GetInstance().getCompile().Assert(CompileMark.NeiMeng))
		{
			...
			{
				sb.append(TabMark);
				sb.append(item.tgsmall[i].GsmNcellCarrierRSSI);
				sb.append(TabMark);
				sb.append(item.tgsmall[i].GsmNcellBcch);
				sb.append(TabMark);
				sb.append(item.tgsmall[i].GsmNcellBsic);
				sb.append(TabMark);
				sb.append(item.tgsmall[i].GsmNcellLac);
				sb.append(TabMark);
				sb.append(item.tgsmall[i].GsmNcellCi);
			}
		}
	...
	{	
		...
		sb.append(item.ispeed);sb.append(TabMark);            
		sb.append(item.imode);sb.append(TabMark);             
		sb.append(item.iLAC);sb.append(TabMark);              
		sb.append(item.iCI);sb.append(TabMark);      
		sb.append(item.Eci);sb.append(TabMark); 
		sb.append(item.getEncryptIMSI());sb.append(TabMark);              
		sb.append(item.getEncryptMSISDN());sb.append(TabMark);            
	...
	public static String getPutLteSample_Temp(DT_Sample_4G item)
	{
		// String samKey = RowKeyMaker.DtLteSampleKey(0, 100, item.IMSI,
		// item.itime, item.wtimems, item.ilongitude, item.ilatitude);
		...
		sb.append(item.LteScPUSCHPRBNum);
		sb.append(TabMark);
		sb.append(item.LteScPDSCHPRBNum);
		sb.append(TabMark);
		sb.append(item.LteSceNBRxTxTimeDiff);

		return sb.toString();

	}

	public static String getPutLteVillageSample(DT_Sample_4G item)
	{
		// String samKey = RowKeyMaker.DtLteSampleKey(0, 100, item.IMSI,
		// item.itime, item.wtimems, item.ilongitude, item.ilatitude);
	...
	public static String getPutCellGrid_2G(Stat_CellGrid_2G item)
	{
		// String samKey = RowKeyMaker.DtLteCellGridKey(0, 100, item.iLac,
		// item.iCi, item.startTime, item.itllongitude, item.itllatitude);
	...
	public static String getPutCellGrid_3G(Stat_CellGrid_3G item)
	{
		// String samKey = RowKeyMaker.DtLteCellGridKey(0, 100, item.iLac,
		// item.iCi, item.startTime, item.itllongitude, item.itllatitude);
