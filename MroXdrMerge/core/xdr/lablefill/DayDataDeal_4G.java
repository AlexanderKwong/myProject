...
import StructData.Stat_Grid_Freq_4G;
...
public class DayDataDeal_4G
{
	...
	// 输出数据, all 为true,则输出所有数据,为false则先判断数据条数再决定输出
	public void outData(boolean all, MultiOutputMng<NullWritable, Text> mosMng, Text curText)
	{
		Map<String, Integer> map = null;

		Collection<DayDataItem> dayDataItems = dayDataDealMap.values();

		if (!all)
		{
			map = new HashMap<String, Integer>();

			for (DayDataItem dayDataItem : dayDataItems)
			{
				dayDataItem.checkCount(map);
			}
			if (map.size() == 0)
				return;
		}
		...
		for (DayDataItem dayDataItem : dayDataItems)
		{
			dayDataItem.outData(mosMng, curText, map);
		}
	}
	...
	public class DayDataItem
	{
		...
		public void checkCount(Map<String, Integer> map)
		{
			checkOneMapCount(map, MrOutGrid.TypeName, mrOutGridMap.size());
			checkOneMapCount(map, MrBuild.TypeName, mrBuildMap.size());
			checkOneMapCount(map, MrInGrid.TypeName, mrInGridMap.size());
			checkOneMapCount(map, MrBuildCell.TypeName, mrBuildCellMap.size());
			checkOneMapCount(map, MrInGridCell.TypeName, mrInGridCellMap.size());
			checkOneMapCount(map, MrOutGridCell.TypeName, mrOutGridCellMap.size());
			checkOneMapCount(map, MrStatCell.TypeName, mrStatCellMap.size());
			checkOneMapCount(map, MrBuildCellNc.TypeName, mrBuildCellNcMap.size());
			checkOneMapCount(map, MrInGridCellNc.TypeName, mrInGridCellNcMap.size());
			checkOneMapCount(map, MrOutGridCellNc.TypeName, mrOutGridCellNcMap.size());
			checkOneMapCount(map, TopicCellIsolated.TypeName, topicCellIsolatedMap.size());
			//////////////////////////////////////////////////////////////////////////////////////
		}

		private void checkOneMapCount(Map<String, Integer> map, String typeName, int count)
		{
			Integer item = map.get(typeName);
			if (item == null)
				item = 0;
			item += count;
		...
		public void outData(MultiOutputMng<NullWritable, Text> mosMng, Text curText, Map<String, Integer> map)
		{
			if (map == null || map.get(MrOutGrid.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrOutGrid.TypeName, mrOutGridMap.values());
				mrOutGridMap.clear();
			}

			if (map == null || map.get(MrBuild.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrBuild.TypeName, mrBuildMap.values());
				mrBuildMap.clear();
			}

			if (map == null || map.get(MrInGrid.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrInGrid.TypeName, mrInGridMap.values());
				mrInGridMap.clear();
			}

			if (map == null || map.get(MrBuildCell.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrBuildCell.TypeName, mrBuildCellMap.values());
				mrBuildCellMap.clear();
			}

			if (map == null || map.get(MrInGridCell.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrInGridCell.TypeName, mrInGridCellMap.values());
				mrInGridCellMap.clear();
			}

			if (map == null || map.get(MrOutGridCell.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrOutGridCell.TypeName, mrOutGridCellMap.values());
				mrOutGridCellMap.clear();
			}

			if (map == null || map.get(MrStatCell.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrStatCell.TypeName, mrStatCellMap.values());
				mrStatCellMap.clear();
			}

			if (map == null || map.get(MrBuildCellNc.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrBuildCellNc.TypeName, mrBuildCellNcMap.values());
				mrBuildCellNcMap.clear();
			}

			if (map == null || map.get(MrInGridCellNc.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrInGridCellNc.TypeName, mrInGridCellNcMap.values());
				mrInGridCellNcMap.clear();
			}

			if (map == null || map.get(MrOutGridCellNc.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, MrOutGridCellNc.TypeName, mrOutGridCellNcMap.values());
				mrOutGridCellNcMap.clear();
			}

			if (map == null || map.get(TopicCellIsolated.TypeName) > 100000)
			{
				outOneMap(mosMng, curText, TopicCellIsolated.TypeName, topicCellIsolatedMap.values());
		...
		private void outOneMap(MultiOutputMng<NullWritable, Text> mosMng, Text curText, String typeName, Collection<IResultTable> items)
		{
			for (IResultTable item : items)
			{
				...
				try
				{
					mosMng.write(typeName, NullWritable.get(), curText);
				}
				catch (Exception e)
				{
					e.printStackTrace();
		...
		public void dealMr(DT_Sample_4G sample)
		{
			if (statType == StatDeal.STATDEAL_ALL)
			{
				...
				if (MainModel.GetInstance().getCompile().Assert(CompileMark.MrStat))
				{
		...
		private void statMrOutGrid(DT_Sample_4G sample)
		{
		...
		private void statMrBuild(DT_Sample_4G sample)
		{
		...
		private void statMrInGrid(DT_Sample_4G sample)
		{
		...
		private void statMrBuildCell(DT_Sample_4G sample)
		{
		...
		private void statMrInGridCell(DT_Sample_4G sample)
		{
		...
		private void statMrOutGridCell(DT_Sample_4G sample)
		{
		...
		private void statMrStatCell(DT_Sample_4G sample)
		{
		...
		private void statTopicCellIsolated(DT_Sample_4G sample)
		{
		...
		private void statCell(DT_Sample_4G sample)
		{
			if (sample.Eci > 0)
			{
				...
				if (sample.flag.toUpperCase().equals("MRO"))
				{
					...
					int val = ImeiConfig.GetInstance().getValue(sample.imeiTac);
					if (itemList.size() > 0)
					{
